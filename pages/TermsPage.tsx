  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#141414] text-gray-900 dark:text-white font-inter transition-colors duration-200">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 md:py-20 text-gray-600 dark:text-gray-300 space-y-8">
        <div>
          <h1 className="text-3xl md:text-5xl font-black text-black dark:text-white mb-4">{t('termsPage.title', { defaultValue: 'Terms of Service' })}</h1>
          <p className="text-sm text-gray-500">{t('termsPage.lastUpdated', { defaultValue: 'Last Updated: August 16, 2026' })}</p>
        </div>

        <section className="space-y-4">
          <h2 className="text-2xl font-bold text-black dark:text-white">{t('termsPage.section1Title', { defaultValue: '1. Acceptance of Terms' })}</h2>
          <p>{t('termsPage.section1Desc', { defaultValue: 'By accessing or using Pstream, you agree to be bound by these Terms of Service. If you do not agree to these terms, you may not use our service.' })}</p>
        </section>
      </main>
    </div>
  );
